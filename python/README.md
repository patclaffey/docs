...
The Python repository provides essential base images to serve as the starting point for Python developers on Docker.  The supported Python releases are 2.7, 3.3, 3.4 and 3.5 which is latest.  All images are built on Debian 8 (code named jessie) except for the apline images which are built on Alpine Linux.
